...
use crate::AppState;
use crate::requests::{CreateSpotRequest, CreateSpotTimestamp ,CreateSpotResponse, Entry};
...
pub async fn create_spots(
    State(state): State<AppState>,
    JsonExtractor(new_spots): JsonExtractor<CreateSpotRequest>,
) -> Result<Json<CreateSpotResponse>, EntryError> {
    tracing::info!("Received new spot: {:?}", new_spots);

    let config = config().await;

    if new_spots.entries.is_empty() {
        return Ok(Json(CreateSpotResponse {
            data_range: CreateSpotTimestamp {
                start_timestamp: new_spots.data_range.start_timestamp,
                end_timestamp: new_spots.data_range.end_timestamp,
            },
            .. Default::default()
        }));
    }

    let publisher_name = new_spots.entries[0].base.publisher.clone();
    ...
    let message_hash = build_publish_message(&new_spots.entries)?.message_hash(account_address);

    if !ecdsa_verify(
        &public_key,
        &message_hash,
        &Signature {
            r: new_spots.signature[0],
            s: new_spots.signature[1],
        },
    )
    ...
    Ok(Json(CreateSpotResponse {
        pair_id: new_spots.pair_id,
        price: new_spots.price,
        data_range: CreateSpotTimestamp {
            start_timestamp: new_spots.data_range.start_timestamp,
            end_timestamp: new_spots.data_range.end_timestamp,
        },
        volume: new_spots.volume,
    }))
}
