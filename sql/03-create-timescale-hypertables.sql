...
CALL add_columnstore_policy('mainnet_spot_entry', after => INTERVAL '12 hours');

ALTER TABLE spot_entry SET (
    timescaledb.enable_columnstore = true,
    timescaledb.segmentby = 'pair_id'
);
CALL add_columnstore_policy('spot_entry', after => INTERVAL '12 hours');

ALTER TABLE mainnet_future_entry SET (
    timescaledb.enable_columnstore = true,
    timescaledb.segmentby = 'pair_id'
);
CALL add_columnstore_policy('mainnet_future_entry', after => INTERVAL '12 hours');

ALTER TABLE future_entry SET (
    timescaledb.enable_columnstore = true,
    timescaledb.segmentby = 'pair_id'
);
CALL add_columnstore_policy('future_entry', after => INTERVAL '12 hours');
