...
use pragma_entities::models::entry::NewStarkexEntry;
use pragma_entities::models::future_entry::NewStarkexFutureEntry;
use pragma_entities::{
...
async fn process_payload(pool: &Pool, payload: Vec<u8>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let is_starkex_entries = decoded_payload.contains("starkex");
    let is_future_entries = decoded_payload.contains("expiration_timestamp");

    if is_future_entries {
        if is_starkex_entries {
            match serde_json::from_slice::<Vec<NewFutureEntry>>(&payload) {
                Ok(future_entries) => {
                    if !future_entries.is_empty() {
                        if let Err(e) = insert_starkex_future_entries(
                            pool,
                            future_entries.into_iter().map(|x| x.into()).collect(),
                        )
                        .await
                        {
                            error!("error while inserting starkex future entries : {:?}", e);
                        }
                    }
                }
                Err(e) => {
                    error!("Failed to deserialize starkex future payload: {:?}", e);
                }
            }
        } else {
            match serde_json::from_slice::<Vec<NewFutureEntry>>(&payload) {
        ...
        if is_starkex_entries {
            match serde_json::from_slice::<Vec<NewEntry>>(&payload) {
                Ok(entries) => {
                    info!(
                        "[STARKEX SPOT] total of '{}' new entries available.",
                        entries.len()
                    );
                    if let Err(e) = insert_starkex_spot_entries(
                        pool,
                        entries.into_iter().map(|x| x.into()).collect(),
                    )
                    .await
                    {
                        error!("error while inserting starkex entries : {:?}", e);
                    }
                }
                Err(e) => {
                    error!("Failed to deserialize starkex payload: {:?}", e);
                }
            }
        } else {
            match serde_json::from_slice::<Vec<NewEntry>>(&payload) {
...
#[tracing::instrument(skip(pool))]
pub async fn insert_starkex_spot_entries(
    pool: &Pool,
    new_entries: Vec<NewStarkexEntry>,
) -> Result<(), InfraError> {
    let conn = pool.get().await.map_err(adapt_infra_error)?;
    let entries = conn
        .interact(move |conn| NewStarkexEntry::create_many(conn, new_entries))
        .await
        .map_err(adapt_infra_error)?
        .map_err(adapt_infra_error)?;
...
#[tracing::instrument(skip(pool))]
pub async fn insert_starkex_future_entries(
    pool: &Pool,
    new_entries: Vec<NewStarkexFutureEntry>,
) -> Result<(), InfraError> {
    let conn = pool.get().await.map_err(adapt_infra_error)?;
    ...
    info!("[STARKEX PERP] {} new entries available", len_perp_entries);
    info!(
        "[STARKEX FUTURE] {} new entries available",
        new_entries.len() - len_perp_entries
    );

    let entries = conn
        .interact(move |conn| NewStarkexFutureEntry::create_many(conn, new_entries))
        .await
        .map_err(adapt_infra_error)?
        .map_err(adapt_infra_error)?;
    ...
    for entry in &entries {
        info!(
            "new starkex future entry created {} - {}({}) - {}",
            entry.publisher, entry.pair_id, entry.price, entry.source
        );
    }
    Ok(())
