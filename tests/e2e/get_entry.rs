...
#[serial_test::serial]
#[tokio::test]
async fn get_entry_twap_routing_many_ok(
    #[future] setup_containers: TestHelper,
    #[case] queried_interval: Interval,
) {
    let mut hlpr = setup_containers.await;
    ...
    let response_routing = reqwest::get(hlpr.endpoint(&endpoint))
        .await
        .expect("Error while fetching data from pragma node");

    let response_routing = response_routing
        .json::<GetEntryResponse>()
        .await
        .expect("Could not retrieve a valid GetEntryResponse");
...
#[serial_test::serial]
#[tokio::test]
async fn get_entry_twap_2hours_strk_eth_ok(
    #[future] setup_containers: TestHelper,
    #[case] queried_interval: Interval,
) {
    let mut hlpr = setup_containers.await;
